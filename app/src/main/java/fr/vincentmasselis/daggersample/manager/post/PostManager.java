...
/**
 * Interface qui définit les méthodes utilisées par un manager pour travailler avec des {@link Post}
 * Quand un object a besoin de faire des appels WS pour récupérer les posts, il doit se faire
 * injecter cette interface (un des principes de base de l'IOC).
 * Cependant, Dagger2 n'est pas capable de deviner quel sera l'implémentation de cette interface,
 * pour ce faire, il faut alors définir un binding disant que l'implémentation actuelle de
 * {@link PostManager} est {@link PostManagerImpl}.
 * Ce "binding" est visible dans {@link fr.vincentmasselis.daggersample.manager.ManagerModule}.
 ...
 * lors de tests unitaires, on peut imaginer utiliser une implémentation différente de
